  ...
  return (
    <div>
      {new Array(maximum).fill("").map((_, index) => (
        <span
          key={index}
          className={`${index < stars ? "text-warning" : "text-muted"}`}
        >
          &#9733;
        </span>
      ))}
            ...
            <div className="row mt-3 mt-lg-0">
              <div className="col-12 col-lg-9 w-100">
                <div className="w-100 video position-relative">
                  ...
                  <iframe
                    id="myIframe"
                    className={clsx("responsive-iframe w-100")}
                    frameBorder="0"
                    src={
                      type == "movie"
                        ? embedMovie(id)
                        : embedEpisode(id, episode.season, episode.episode)
                    }
                    allowFullScreen
                  ></iframe>
                </div>
                <div className="infoWatch mt-3">
                  ...
                  <ul className="mb-1">
                    {movie.genres.map((genre) => (
                      <Link
                        to={{
                          pathname: `/genre/${genre.id}-${urlToSlug(
                            genre.name
                          )}/${type}`,
                          query: {
                            name: genre.name,
                  ...
                  {type == "tv" && (
                    <h5>
                      {
                        movie.seasons[episode.indexSeason].episodes[
              ...
              <div className="col-12 col-lg-3 w-100">
                {type === "movie" ? (
                    ...
                    <div className="similars d-flex flex-column pr-2">
                      {movie.similar.results.map((item) => (
                        <div className="item d-flex" key={item.id}>
                          <Link
                            to={`/movie/${
                              item.id + "-" + urlToSlug(item.title)
                            }`}
                          >
                            <div className="poster">
                              <img
                                className="img-fluid easeload"
                                width={100}
                                ...
                                onLoad={({ currentTarget }) => {
                                  ...
                                  currentTarget.style.transform =
                                    currentTarget.src.includes("no_poster") &&
                                    "scale(0.5)";
                          ...
                          <div className="d-flex flex-column">
                            <Link
                              title={item.title}
                              className={clsx(
                                "line-clamp-2 font-weight-bold titleSimilar",
                                id == item.id && "text-orange"
                              )}
                              to={`/movie/${
                                item.id + "-" + urlToSlug(item.title)
                              }`}
                            ...
                            <StarRating
                              ...
                              extraText={` (${item.vote_count.toLocaleString()} votes)`}
                            />
                            <Link
                              to={`/movie/${
                                item.id + "-" + urlToSlug(item.title)
                  ...
                  <>
                    ...
                    >
                      <Link to={`/tv/${id}-${title}/seasons`}>Seasons</Link>
                    </h4>
                        ...
                        <Fragment key={item.season_number}>
                          ...
                          >
                            ...
                            <div className="info">
                              ...
                              <p className="mb-0">
                                <span>{item.episodes.length}</span> Episode
                                {item.episodes.length === 1 ? "" : "s"}
                              </p>
                              <p className="mb-0">
                                {handleDate(item.air_date)}
                              </p>
                            </div>
                          </div>
